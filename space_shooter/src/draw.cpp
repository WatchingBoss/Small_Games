...
static bool HS_INS = 0, ES_INS[ES_NUM] = { 0 }, HB_INS[HB_NUM] = { 0 },
            EB_INS[EB_NUM] = { 0 };
...
static inline void blaster_return( bool &b_shoot, glm::vec3 &b_pos ) {
	b_shoot  = false;
	b_pos[0] = 0;
	b_pos[1] = 0;
}

static inline void intersection_occur( bool &b_ins, bool &b_shoot, bool &s_ins,
                                       glm::vec3 &b_pos ) {
	b_ins = true;
	s_ins = true;
...
void DrawHeroShip( const Renderer &rend, GameObject<glm::vec3, HB_NUM> *hero ) {
	/* SHIP */
	...
	/* BLASTERS */
	{
		...
		for ( size_t i = 0; i < HB_NUM; ++i ) {
			if ( HB_shoot[i] ) {
				...
				if ( !hero && cb_pos[1] < 0 ) blaster_return( HB_shoot[i], cb_pos );
			}
		}
...
void let_enemy_shoot( std::atomic<bool> &run ) {
...
void DrawEnemyShip( const Renderer &                                   rend,
	...
	for ( int i = ES_NUM - 1; i >= 0; --i ) {
		if ( !ES_INS[i] && ( ( i > ( ES_NUM - ES_COL_NUM ) ) ||
...
void Check_Intersection( GameObject<glm::vec3, HB_NUM> *                    hero,
	...
	for ( size_t hb = 0; hb < HB_NUM; ++hb ) {
		if ( !HB_shoot[hb] ) continue;

		for ( size_t es = 0; es < ES_NUM; ++es ) {
			if ( ES_INS[es] ) continue;
			...
			if ( ( ( es_pos[0] <= leftV[0] && es_pos[0] + SHIP_HEIGHT >= leftV[0] &&
			       ...
			       ( es_pos[0] <= rightV[0] && es_pos[0] + SHIP_HEIGHT >= rightV[0] &&
			         es_pos[1] == rightV[1] ) ) ||
			     ( es_pos[0] + SHIP_HEIGHT == leftV[0] && es_pos[1] <= leftV[1] &&
			       es_pos[1] + SHIP_HEIGHT >= leftV[1] ) ||
				...
				if ( countDestroyEnemyShip == ES_NUM ) gameWin = true;
			}
		}
	}

	for ( size_t eb = 0; eb < EB_NUM; ++eb ) {
		if ( !EB_shoot[eb] ) continue;

		const glm::vec3 &eb_pos   = eb_position.at( eb );
		const float      leftV[2] = { eb_pos[0], eb_pos[1] - BLASTER_HEIGHT };
		const float rightV[2] = { eb_pos[0] + BLASTER_WIDTH, eb_pos[1] - BLASTER_HEIGHT };
		...
		if ( ( ( hs_pos[1] <= leftV[1] && leftV[1] <= hs_pos[1] + SHIP_HEIGHT &&
		     ...
		     ( hs_pos[0] == rightV[0] && hs_pos[1] <= rightV[1] &&
		       hs_pos[1] + SHIP_HEIGHT >= rightV[1] ) ||
		     ( hs_pos[0] + SHIP_WIDTH == leftV[0] && hs_pos[1] <= leftV[1] &&
		       hs_pos[1] + SHIP_HEIGHT >= leftV[1] ) )
			gameOver = true;
	}
}

static float circles = 0, circles_chang = 1.f, center_chang = 0.05f;
void         DrawBackStars( const Renderer *rend, BackStar *star ) {
    star->tex.Bind( 0 );
    ...
    for ( size_t i = 0; i < STARS_NUM; ++i ) {
        ...
        float &    center_x  = star->dim.center.at( i ).at( 0 ),
        ...
        star->shader.SetUniformMatrix4fv( "model", 1, GL_FALSE, glm::value_ptr( model ) );
...
void EndOfGame( const Renderer *rend, EndText *text, const bool &win ) {
